...
export default function BookDetailPage() {
  ...
  return (
    <div className="min-h-screen">
      <Header title={bookName} subtitle={`Coverage as of Century ${toRoman(century)}`} />
          ...
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex gap-2">
              {[undefined, "papyrus", "uncial"].map((t) => (
                ...
                >
                  {t === undefined ? "All" : t === "papyrus" ? "Papyri" : "Uncials"}
                </button>
              ))}
            </div>
          </div>
              ...
              {timelineData && (
                <div className="rounded-xl border border-border bg-card p-6">
                  <h3 className="mb-3 text-sm font-semibold">
                    Coverage Timeline &mdash; {bookName}
                  </h3>
            ...
            {missingData && missingData.totalMissing > 0 && (
              <div className="rounded-xl border border-border bg-card p-6">
                <h3 className="mb-1 text-sm font-semibold">
                  Missing Verses ({missingData.totalMissing})
                </h3>
                <p className="mb-3 text-xs text-muted-foreground">
                  Verses not attested by any manuscript up to century{" "}
                  {toRoman(century)}
                </p>
