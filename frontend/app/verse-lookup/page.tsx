...
function VerseLookupContent() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!parsed) {
      setError("Referência inválida. Use sigla em português (Mt, 1 Co) ou inglês (Matthew, 1Cor), ex.: Mt 1:1 ou John 18:31");
      setResult(null);
      setSearched(true);
      return;
    }
  ...
  return (
    <div className="min-h-screen">
      <Header
        title="Consulta por versículo"
        subtitle="Versículo → Manuscritos"
      />

      <div className="p-6 space-y-6">
        <div className="rounded-xl border border-border bg-card p-6">
          <p className="text-sm text-muted-foreground mb-4">
            Digite a referência com sigla em português (Mt, 1 Co) ou inglês (Matthew, 1Cor). Ex.: Mt 1:1, Jo 18:31, 1 Co 13:1
          </p>
          <form onSubmit={handleSubmit} className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <div className="flex-1">
              ...
              <input
                ...
                placeholder="Mt 1:1 ou John 18:31"
            ...
            <div className="flex gap-2 flex-wrap items-center">
              {[undefined, "papyrus", "uncial"].map((t) => (
                <button
                  key={t ?? "all"}
                  type="button"
                  onClick={() => setType(t)}
                ...
                >
                  {t === undefined ? "Todos" : t === "papyrus" ? "Papiros" : "Unciais"}
                </button>
              ))}
              <button
                type="submit"
          ...
          <div className="rounded-xl border border-border bg-card overflow-hidden">
            <div className="px-6 py-4 border-b border-border bg-muted/30">
              ...
              <p className="text-sm text-muted-foreground">
                {result.manuscripts.length} manuscrito(s) que contêm este versículo
              </p>
            </div>
            {result.manuscripts.length === 0 ? (
              <div className="p-12 text-center text-muted-foreground">
                Nenhum manuscrito encontrado para este versículo.
              </div>
            ) : (
                ...
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border bg-muted/50">
                      ...
                      <th className="px-4 py-3 text-left text-sm font-medium">Século</th>
        ...
        {searched && !loading && !error && !result && (
          <div className="rounded-xl border border-border bg-card p-12 text-center text-muted-foreground">
            Digite uma referência e clique em Buscar.
          </div>
        )}
      </div>
    </div>
