...
export default function ManuscriptDetailPage() {
  ...
  return (
    <div className="min-h-screen">
      <Header
        title={gaId}
        subtitle={data?.name ?? "Manuscript details"}
      />
          ...
          <div className="space-y-6">
            <div className="rounded-xl border border-border bg-card p-6">
              <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
                <div>
                  <p className="text-xs text-muted-foreground">GA ID</p>
                  <p className="font-mono font-bold">{data.gaId}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground">Century</p>
                  <p>
              ...
              {data.historicalNotes && (
                <div className="mt-4 pt-4 border-t border-border">
                  <p className="text-xs text-muted-foreground mb-2">Historical Notes</p>
              ...
              <div className="mt-4 pt-4 border-t border-border">
                ...
                >
                  View in NTVMR <ExternalLink className="h-4 w-4" />
                </a>
              </div>
            </div>

            <div className="rounded-xl border border-border bg-card p-6">
              <h3 className="text-base font-semibold mb-4">Books Preserved</h3>
              <div className="space-y-4">
                {data.booksPreserved.map((br) => (
                  <div key={br.book} className="border-b border-border last:border-0 pb-4 last:pb-0">
                    <p className="font-medium">{br.book}</p>
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              <h3 className="text-base font-semibold mb-4">Intervals Summary</h3>
              <div className="overflow-x-auto">
                <table className="w-full">
                  <thead>
                    <tr className="border-b border-border">
                      <th className="px-4 py-2 text-left text-sm font-medium">Book</th>
                      <th className="px-4 py-2 text-left text-sm font-medium">Chapters</th>
                      <th className="px-4 py-2 text-right text-sm font-medium">Verses</th>
                    </tr>
                  </thead>
                  <tbody>
                    {data.intervals.map((i) => (
                      <tr key={i.book} className="border-b border-border last:border-0">
                        <td className="px-4 py-2">{i.book}</td>
