...
class IngestionPhaseRepository {
    ...
    fun getByPrefix(prefix: String): List<PhaseStatusDTO> = transaction {
        CouncilIngestionPhases.selectAll()
            .where { CouncilIngestionPhases.phaseName like "$prefix%" }
            .orderBy(CouncilIngestionPhases.phaseName to SortOrder.ASC)
            .map { it.toDto() }
    }
    ...
    fun deleteByPrefix(prefix: String): Int = transaction {
        CouncilIngestionPhases.deleteWhere { phaseName like "$prefix%" }
    }

    fun isAnyRunningByPrefix(prefix: String): Boolean = transaction {
        CouncilIngestionPhases.selectAll()
            .where { (CouncilIngestionPhases.status eq "running") and (CouncilIngestionPhases.phaseName like "$prefix%") }
            .count() > 0
    }

    fun isPhaseRunning(phaseName: String): Boolean = transaction {
        CouncilIngestionPhases.selectAll()
            .where { (CouncilIngestionPhases.phaseName eq phaseName) and (CouncilIngestionPhases.status eq "running") }
            .count() > 0
    }
