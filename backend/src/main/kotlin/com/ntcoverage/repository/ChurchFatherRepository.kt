...
class ChurchFatherRepository {
    ...
    fun countByTradition(): Map<String, Int> = transaction {
        ChurchFathers
            .select(ChurchFathers.tradition, ChurchFathers.id.count())
            .groupBy(ChurchFathers.tradition)
            .associate { it[ChurchFathers.tradition] to it[ChurchFathers.id.count()].toInt() }
    }

    fun countByCentury(): Map<Int, Int> = transaction {
        ChurchFathers
            .select(ChurchFathers.centuryMin, ChurchFathers.id.count())
            .groupBy(ChurchFathers.centuryMin)
            .associate { it[ChurchFathers.centuryMin] to it[ChurchFathers.id.count()].toInt() }
    }

    fun countWithDating(): Int = transaction {
        ChurchFathers.selectAll()
            .where { ChurchFathers.yearBest.isNotNull() }
            .count().toInt()
    }
    ...
    fun updateBiographySummary(fatherId: Int, summary: String): Boolean = transaction {
        val updated = ChurchFathers.update({ ChurchFathers.id eq fatherId }) {
            it[biographySummary] = summary
            it[updatedAt] = OffsetDateTime.now()
        }
        updated > 0
    }
