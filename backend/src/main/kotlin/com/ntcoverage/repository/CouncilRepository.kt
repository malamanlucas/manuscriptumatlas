...
class CouncilRepository {
    ...
    fun search(query: String, limit: Int = 20, locale: String = "en"): List<CouncilSummaryDTO> = transaction {
        ...
        val selectSql = if (locale == "en") {
            ...
            OR unaccent(lower(councils.display_name)) LIKE unaccent(lower('%$escaped%'))
            ...
            OR unaccent(lower(c.display_name)) LIKE unaccent(lower('%$escaped%'))
            OR unaccent(lower(coalesce(ct.display_name,''))) LIKE unaccent(lower('%$escaped%'))
    ...
    fun updateConflictResolution(
        ...
        Councils.update({ Councils.id eq councilId }) {
            when (field) {
                "year" -> value.toIntOrNull()?.let { v -> it[Councils.year] = v }
                "yearEnd" -> it[Councils.yearEnd] = value.toIntOrNull()
                "location" -> it[Councils.location] = value
                "participants" -> it[Councils.numberOfParticipants] = value.toIntOrNull()
            }
            it[Councils.conflictResolution] = justification
            it[consensusConfidence] = confidence
            it[updatedAt] = OffsetDateTime.now()
        } > 0
    }
