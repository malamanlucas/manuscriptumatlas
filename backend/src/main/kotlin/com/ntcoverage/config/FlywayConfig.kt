    ...
    private fun createTablesViaExposed() {
        transaction {
            SchemaUtils.createMissingTablesAndColumns(
                ...
                CouncilHereticParticipants, VisitorDailyStats, Users, LlmUsageLogs, LlmPromptQueue,
                ApologeticTopics, ApologeticResponses, ApologeticTopicTranslations, ApologeticResponseTranslations
            )
        ...
        transaction {
            ...
            exec("""
                ...
                BEGIN
                    ...
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_llm_usage_logs_created_at') THEN
                        CREATE INDEX idx_llm_usage_logs_created_at ON llm_usage_logs(created_at DESC);
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_llm_usage_logs_provider') THEN
                        CREATE INDEX idx_llm_usage_logs_provider ON llm_usage_logs(provider);
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_apologetic_topics_title_trgm') THEN
                        CREATE INDEX idx_apologetic_topics_title_trgm ON apologetic_topics USING gin (title gin_trgm_ops);
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_apologetic_topics_fts') THEN
                        CREATE INDEX idx_apologetic_topics_fts ON apologetic_topics USING gin (
                            to_tsvector('english', coalesce(title,'') || ' ' || coalesce(body,''))
                        );
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_apologetic_topics_status') THEN
                        CREATE INDEX idx_apologetic_topics_status ON apologetic_topics(status);
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_apologetic_responses_topic') THEN
                        CREATE INDEX idx_apologetic_responses_topic ON apologetic_responses(topic_id);
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_apologetic_topic_translations_lookup') THEN
                        CREATE INDEX idx_apologetic_topic_translations_lookup ON apologetic_topic_translations(locale, topic_id);
                    END IF;
                    IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_apologetic_response_translations_lookup') THEN
