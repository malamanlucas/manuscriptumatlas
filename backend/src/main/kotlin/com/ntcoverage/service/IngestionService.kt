...
class IngestionService(
    ...
    companion object {
        val ALL_PHASES = listOf("manuscript_seed_books", "manuscript_ingest", "manuscript_coverage", "manuscript_enrich_dating")
    }

    suspend fun runPhase(phase: String) {
        when (phase) {
            "manuscript_seed_books" -> seedBooksAndVerses()
            "manuscript_ingest" -> ingestManuscriptsAsync()
            "manuscript_coverage" -> materializeCoverageAsync()
            "manuscript_enrich_dating" -> runEnrichDating()
        }
    }

    suspend fun runAllPhases(skipCompleted: Boolean = true) {
        val phases = ALL_PHASES.let { all ->
            if (skipCompleted) {
                val skipped = all.filter { phaseTracker.isPhaseCompleted(it) }
                if (skipped.isNotEmpty()) {
                    log.info("MANUSCRIPT_INGESTION: skipping {} already completed phases: {}", skipped.size, skipped)
                }
                all.filterNot { phaseTracker.isPhaseCompleted(it) }
            } else all
        }
        if (phases.isEmpty()) {
            log.info("MANUSCRIPT_INGESTION: all phases already completed, nothing to run")
            return
        }
        for (phase in phases) {
            runPhase(phase)
    ...
    fun seedBooksAndVerses() {
        val books = CanonicalVerses.books
        phaseTracker.markRunning("manuscript_seed_books", total = books.size, runBy = "system")
        try {
            log.info("Seeding ${books.size} books and ${CanonicalVerses.totalVerses} canonical verses...")
            ...
            for ((index, book) in books.withIndex()) {
                ...
                phaseTracker.markProgress("manuscript_seed_books", index + 1)
            }

            log.info("Canonical seed complete.")
            phaseTracker.markSuccess("manuscript_seed_books", books.size)
        } catch (e: Exception) {
            phaseTracker.markFailed("manuscript_seed_books", e.message ?: "Unknown error")
            throw e
        }
    }

    suspend fun ingestManuscriptsAsync(): IngestionResult {
        phaseTracker.markRunning("manuscript_ingest", runBy = "system")
        try {
            ...
            val manuscripts = if (IngestionConfig.loadManuscriptsFromNtvmr) {
                ...
                if (fetched.isEmpty()) {
                    ...
                    phaseTracker.markSuccess("manuscript_ingest", 0)
            ...
            phaseTracker.markProgress("manuscript_ingest", 0, manuscripts.size)
            ...
            phaseTracker.markSuccess("manuscript_ingest", result.manuscriptsIngested)
            return result
        } catch (e: Exception) {
            phaseTracker.markFailed("manuscript_ingest", e.message ?: "Unknown error")
            throw e
        }
    }

    suspend fun materializeCoverageAsync() {
        phaseTracker.markRunning("manuscript_coverage", total = 10, runBy = "system")
        try {
            ...
            for (century in 1..10) {
                ...
                phaseTracker.markProgress("manuscript_coverage", century)
            }
            ...
            phaseTracker.markSuccess("manuscript_coverage", 10)
        } catch (e: Exception) {
            phaseTracker.markFailed("manuscript_coverage", e.message ?: "Unknown error")
            throw e
        }
    }
    ...
    private fun runEnrichDating(): Int {
        if (!datingEnrichmentService.isEnabled()) {
            log.info("MANUSCRIPT_ENRICH_DATING: dating enrichment disabled — skipping")
            phaseTracker.markRunning("manuscript_enrich_dating", total = 0, runBy = "system")
            phaseTracker.markSuccess("manuscript_enrich_dating", 0)
            return 0
        }

        phaseTracker.markRunning("manuscript_enrich_dating", total = 0, runBy = "system")
        try {
            val enqueued = datingEnrichmentService.enqueueManuscriptDating(limit = 100)
            log.info("MANUSCRIPT_ENRICH_DATING: enqueued $enqueued manuscripts for LLM queue processing")
            phaseTracker.markSuccess("manuscript_enrich_dating", enqueued)
            return enqueued
        } catch (e: Exception) {
            phaseTracker.markFailed("manuscript_enrich_dating", e.message ?: "Unknown error")
            throw e
        }
    }
        ...
        try {
            for (ms in manuscripts) {
                ...
                if (!ntvmrAvailable) {
                    ...
                    processedCount++
                    if (processedCount % 10 == 0) phaseTracker.markProgress("manuscript_ingest", processedCount)
                    continue
                }
