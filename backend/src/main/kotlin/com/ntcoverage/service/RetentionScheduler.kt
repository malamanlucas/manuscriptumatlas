...
class RetentionScheduler(private val dataSource: DataSource) {
    ...
    private fun cleanAppliedQueueItems(retentionDays: Int) {
        val cutoff = OffsetDateTime.now(ZoneOffset.UTC).minusDays(retentionDays.toLong())
        transaction {
            val deleted = LlmPromptQueue.deleteWhere {
                (LlmPromptQueue.status eq "applied") and (LlmPromptQueue.processedAt less cutoff)
            }
            if (deleted > 0) {
                log.info("RETENTION: deleted $deleted applied LLM queue items older than $retentionDays days")
            }
        }
    }
