...
class ChurchFatherService(
    ...
    fun getPatristicStats(): PatristicStatsDTO {
        val totalFathers = repository.countAll()
        val totalStatements = statementRepository.countAll()
        val totalWithDating = repository.countWithDating()
        val byTradition = repository.countByTradition()
        val byCentury = repository.countByCentury().mapKeys { it.key.toString() }
        val topicsSummary = statementRepository.countByTopic()
        return PatristicStatsDTO(
            totalFathers = totalFathers,
            totalStatements = totalStatements,
            totalWithDating = totalWithDating,
            byTradition = byTradition,
            byCentury = byCentury,
