...
class SourceConsensusEngine(
    ...
    suspend fun calculateConsensus(councilId: Int, councilName: String = "Unknown council"): ConsensusResult {
        ...
        if ((yearConsensus?.confidence ?: 0.0) < 0.70 && claims.size >= 2) {
            val resolution = conflictResolver.resolveConflictSync(
                councilId = councilId,
                claims = claims,
    ...
    fun calculateConsensusAndEnqueue(councilId: Int, councilName: String = "Unknown council"): Pair<ConsensusResult, Int> {
        val claims = claimRepository.findByCouncil(councilId)
        if (claims.isEmpty()) {
            return ConsensusResult(confidenceScore = 0.0, dataConfidence = "LOW", sourceCount = 0) to 0
        }
        ...
        var enqueued = 0
        if ((yearConsensus?.confidence ?: 0.0) < 0.70 && claims.size >= 2) {
            val id = conflictResolver.enqueueConflict(
                councilId = councilId,
                claims = claims,
                sources = sourceRepository.findAll(),
                field = "year",
        ...
        val dataConfidence = when {
            baseConfidence >= 0.85 -> "HIGH"
            baseConfidence >= 0.60 -> "MEDIUM"
            else -> "LOW"
        }

        val result = ConsensusResult(
            consensusYear = yearConsensus?.value,
            consensusYearEnd = yearEndConsensus?.value,
            consensusLocation = locationConsensus?.value,
            consensusParticipants = participantsConsensus?.value,
            ...
            dataConfidence = dataConfidence,
            sourceCount = claims.size,
            conflictResolution = if (enqueued > 0) "Awaiting LLM resolution" else null
        )

        return result to enqueued
    }
